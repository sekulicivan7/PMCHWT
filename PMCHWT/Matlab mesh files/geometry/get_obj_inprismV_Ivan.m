...
function obj_inpV = get_obj_inprismV_Ivan( obj , obj_V, loop_or , fact_vl )

check_border_cross = 1;

obj_inpV = struct('vertex',[],'topol_Qpr',[],'topol_tet_1',[],'topol_tet_2',[],'topol_tet_3',[],'topol_Qpr_T1',[],'topol_Qpr_T2',[],'topol_Spr',[],'ind_Sc',[],'un_int_Qpr_T1',[],'un_int_Qpr_T2',[],'un_int_Spr',[],'ds_Qpr_T1',[],'ds_Qpr_T2',[],'ds_Spr',[],'ln_Qpr',[],'fact',[],'Hpr',[],'un_nrm_v',[],'un_nrm_tri_v',[],'invrt_to_ed',[],'cent_Spr',[]);

obj_inpV.fact = fact_vl;
...
for m=1:Nv,
    ...
    obj_inpV.un_nrm_v(:,m) = unitary(un_tmp);
    
end; %% for m=1:Nv,

%%% SETUP %%%
obj_inpV.vertex = zeros(3,3*Nt);
obj_inpV.Hpr = zeros(1,Nt);

obj_inpV.topol_Qpr = zeros(4,3*Nt);
obj_inpV.topol_Qpr_T1 = zeros(3,3*Nt);
obj_inpV.topol_Qpr_T2 = zeros(3,3*Nt);
obj_inpV.topol_Spr = zeros(3,Nt);

obj_inpV.ind_Sc = zeros(2,Ne);
obj_inpV.ln_Qpr = zeros(1,3*Nt);
obj_inpV.invrt_to_ed = zeros(1,3*Nt);

obj_inpV.topol_tet_1 = zeros(4,Nt);
obj_inpV.topol_tet_2 = zeros(4,Nt);
obj_inpV.topol_tet_3 = zeros(4,Nt);

%%%  new forget_obj_inprismV_2
%%% >>>
%%% >>>
obj_inpV.un_nrm_tri_v = zeros( 3 , 3*Nt);
obj_inpV.cent_Spr = zeros( 3 , Nt);
%%% <<<
%%% <<<
...
for p=1:Nt,
    ...
    for n=1:3,
        ...
        obj_inpV.un_nrm_tri_v(:, 3*(p-1) + n ) = vec_nrm ;
        %%%% <<<
        %%%% <<<
        
        %%%% cos_ang_p_n = sum( un_p .* obj_inpV.un_nrm_v(:,v_S(n) ));
        ...
        %%%% vert_p_n = obj.vertex(:,v_S(n)) - H_p_n * obj_inpV.un_nrm_v(:,v_S(n) );
        %%%% modified for get_obj_inprismV_2
        %%%% >>>
        ...
        obj_inpV.vertex(:,3*(p-1) + n) = vert_p_n;
        
        %%% Find and Update the distance between the top vertex with the bottom vertex
        obj_inpV.ln_Qpr(3*(p-1) + n) = norm( vert_p_n - obj.vertex(:,v_S(n)) );
        
        %%% Update the centroid of the bottom triangle
        obj_inpV.cent_Spr(:,p) = obj_inpV.cent_Spr(:,p) + vert_p_n/3;
        ...
        obj_inpV.ind_Sc(i_n,ed_n) = 3*(p-1) + n;
        obj_inpV.invrt_to_ed(3*(p-1) + n) = ed_n;
        
    end; %%% for n=1:3,
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% (V)     update of the vertex information in obj_inpV.topol_Qpr,  %%
    %% obj_inpV.topol_Qpr_T1, obj_inpV.topol_Qpr_T2, obj_inpV.topol_Spr %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    obj_inpV.topol_Qpr(:,3*(p-1) + 1) =  [  v_S(2) -(n0+2)  -(n0+3)  v_S(3) ].';
    obj_inpV.topol_Qpr(:,3*(p-1) + 2) =  [  v_S(3) -(n0+3)  -(n0+1)  v_S(1) ].';
    obj_inpV.topol_Qpr(:,3*(p-1) + 3) =  [  v_S(1) -(n0+1)  -(n0+2)  v_S(2) ].';
    
    %%% (b) Triangle T1 of the prism rectangular faces, (pseudo-)NORMAL to
    %%% the surface
    obj_inpV.topol_Qpr_T1(:,3*(p-1) + 1) = [ v_S(2) -(n0+2)  v_S(3) ].';
    obj_inpV.topol_Qpr_T1(:,3*(p-1) + 2) = [ v_S(3) -(n0+3)  v_S(1) ].';
    obj_inpV.topol_Qpr_T1(:,3*(p-1) + 3) = [ v_S(1) -(n0+1)  v_S(2) ].';
    
    %%% (c) Triangle T2 of the prism rectangular faces, (pseudo-)NORMAL to
    %%% the surface
    obj_inpV.topol_Qpr_T2(:,3*(p-1) + 1) = [ -(n0+2)  -(n0+3)  v_S(3) ].';
    obj_inpV.topol_Qpr_T2(:,3*(p-1) + 2) = [ -(n0+3)  -(n0+1)  v_S(1) ].';
    obj_inpV.topol_Qpr_T2(:,3*(p-1) + 3) = [ -(n0+1)  -(n0+2)  v_S(2) ].';
    
    %%% (d) Prism basis
    obj_inpV.topol_Spr(:,p) = [ -(n0+1) -(n0+2) -(n0+3) ].';
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% (VI) Define the THREE tetrahedra that compose the prism (4 vertices involved for each) %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    obj_inpV.topol_tet_1(:,p) = [ v_S(1)  v_S(2)  v_S(3) -(n0+1) ].';
    obj_inpV.topol_tet_2(:,p) = [ v_S(2)  v_S(3) -(n0+1) -(n0+2) ].';
    obj_inpV.topol_tet_3(:,p) = [ v_S(3) -(n0+1) -(n0+2) -(n0+3) ].';
    
end; %%% for p=1:Nt,

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% (VII) Check whether the normal vectors in obj_inpV.un_nrm_tri_v are  %%%%
...
if check_border_cross,
    ...
    for p=1:Nt,
        ...
        for n=1:3,
            ...
            %%% Each vertex check %%%
            %%%%%%%%%%%%%%%%%%%%%%%%%
            cent_p_bottom = obj_inpV.cent_Spr(:,p);
            un_p_n = obj_inpV.un_nrm_tri_v( : , 3*(p-1) + n );  %%% (I)
            
            %%% Establish the vector_coordinates of the mid-plane
            %%% tt_vec = unitary( obj.vertex(:,v_p_S(n)) - cent_p );
            %%% nn_vec = cross( tt_vec , un_p );
            tt_1_vec = unitary( obj_inpV.vertex( : , 3*(p-1) + n ) -  cent_p );
            tt_2_vec = unitary( obj.vertex(:,v_p_S(n)) -  cent_p_bottom );
            
            nn_vec = unitary( cross(tt_1_vec, tt_2_vec ) );
            
            %% projection on the plane of obj_inpV.un_nrm_v(:,v_p_S(n))
            d_un_nrm_prj = sum( obj_inpV.un_nrm_v(:,v_p_S(n)) .* nn_vec );
            un_nrm_v_prj = unitary( obj_inpV.un_nrm_v(:,v_p_S(n)) - d_un_nrm_prj * nn_vec );
            ...
            if ( cos_ang_ref > cos_ang_un_v ),
                ...
                %%% The normal vectors in obj_inpV.un_nrm_tri_v( : , 3*(p-1) + [1:3] )
                ...
                if (cos_ang_un_v > +eps),
                    ...
                    obj_inpV.un_nrm_tri_v( : , 3*(p-1) + n ) = un_nrm_v_prj; %%% (II)
                ...
                vert_p_n_0 = obj_inpV.vertex( : , 3*(p-1) + n );
                
                vert_p_n = obj.vertex(:,v_p_S(n)) - H_p_n * un_nrm_v_prj;
                obj_inpV.vertex( : , 3*(p-1) + n ) = vert_p_n;
                obj_inpV.ln_Qpr( 3*(p-1) + n ) = norm( vert_p_n - obj.vertex(:,v_p_S(n)) );
                ...
                np1 = mod( (n-1) + 1 , 3 ) + 1;
                vert_p_np1_0 = obj_inpV.vertex( : , 3*(p-1) + np1 );
                l_vec_or_np1 = vert_p_np1_0 - cent_p_bottom;
                vert_p_np1 = cent_p_bottom  +  scl_n * l_vec_or_np1;
                obj_inpV.vertex( : , 3*(p-1) + np1 ) = vert_p_np1;
                vec_tmp_np1 = obj.vertex(:,v_p_S(np1)) - vert_p_np1;
                obj_inpV.un_nrm_tri_v( : , 3*(p-1) + np1 ) = unitary( vec_tmp_np1 );  %%% (III)
                obj_inpV.ln_Qpr(3*(p-1) + np1) = norm( vec_tmp_np1 );
                
                %%% [n-1] vertex - correction
                nm1 = mod( (n-1) - 1 , 3 ) + 1;
                vert_p_nm1_0 = obj_inpV.vertex( : , 3*(p-1) + nm1 );
                l_vec_or_nm1 = vert_p_nm1_0 - cent_p_bottom;
                vert_p_nm1 = cent_p_bottom  +  scl_n * l_vec_or_nm1;
                obj_inpV.vertex( : , 3*(p-1) + nm1 ) = vert_p_nm1;
                vec_tmp_nm1 = obj.vertex(:,v_p_S(nm1)) - vert_p_nm1;
                obj_inpV.un_nrm_tri_v( : , 3*(p-1) + nm1 ) = unitary( vec_tmp_nm1 );   %%% (IV)
                obj_inpV.ln_Qpr(3*(p-1) + nm1) = norm( vec_tmp_nm1 );
                
                %%% update the centroid at the bottom triangle
                obj_inpV.cent_Spr(:,p) =  ( vert_p_n + vert_p_np1 + vert_p_nm1 )./ 3;
...
%% Setups
obj_inpV.un_int_Qpr_T1 = zeros(3,3*Nt);
obj_inpV.un_int_Qpr_T2 = zeros(3,3*Nt);
obj_inpV.un_int_Spr = zeros(3,Nt);
obj_inpV.ds_Qpr_T1 = zeros(1,3*Nt);
obj_inpV.ds_Qpr_T2 = zeros(1,3*Nt);
obj_inpV.ds_Spr = zeros(1,Nt);
...
for p=1:Nt,
    ...
    for n=1:3,
        v1 = obj.vertex( : , obj_inpV.topol_Qpr_T1( 1 , 3*(p-1) + n ) );
        v2 = obj_inpV.vertex( :  , abs( obj_inpV.topol_Qpr_T1( 2 , 3*(p-1) + n ) ) );
        v3 = obj.vertex( : , obj_inpV.topol_Qpr_T1( 3, 3*(p-1) + n ) );
        c    = cross(v3-v1,v2-v1);
        obj_inpV.un_int_Qpr_T1( : , 3*(p-1) + n) = unitary(c);
        obj_inpV.ds_Qpr_T1(3*(p-1) + n)   = sqrt(sum(c.^2))/2;
    end; %%% for n=1:3,
    
    %%% (b) Triangle T2 of the (pseudo-)NORMAL quadrilaterals
    for n=1:3,
        v1 = obj_inpV.vertex( : , abs(obj_inpV.topol_Qpr_T2( 1 , 3*(p-1) + n )) );
        v2 = obj_inpV.vertex( :  , abs(obj_inpV.topol_Qpr_T2(2,3*(p-1) + n)) );
        v3 = obj.vertex( : , obj_inpV.topol_Qpr_T2( 3, 3*(p-1) + n ) );
        c    = cross(v3-v1,v2-v1);
        obj_inpV.un_int_Qpr_T2( : , 3*(p-1) + n) = unitary(c);
        obj_inpV.ds_Qpr_T2(3*(p-1) + n)   = sqrt(sum(c.^2))/2;
    end; %%% for n=1:3,

    %%% (c) Opposed Triangle (parallel) to the surface triangle
    v1 = obj_inpV.vertex( : , abs( obj_inpV.topol_Spr( 1 , p) ) );
    v2 = obj_inpV.vertex( :  , abs( obj_inpV.topol_Spr( 2 ,p) ) );
    v3 = obj_inpV.vertex( : , abs( obj_inpV.topol_Spr( 3, p) ) );
    c    = cross( v3-v1 , v2-v1 );
    obj_inpV.un_int_Spr( : , p ) = unitary( c );
    obj_inpV.ds_Spr(p)   = sqrt(sum(c.^2))/2;
